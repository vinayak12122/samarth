...
async def run_page(page,train_number):
    ...
    try:
        train_heading_xpath = f"//div[contains(@class,'train-heading')]//strong[contains(text(), '{train_number}')]"
        
        await page.wait_for_selector(train_heading_xpath,timeout=15000)
        train_box = page.locator("div.bull-back").filter(
            has=page.locator(f"strong:has-text('({train_number})')")
        )
        ...
        try:
            train_box = page.locator("div.tou-mod").filter(has=page.locator(f"strong:has-text('{train_number}')"))
            await train_box.scroll_into_view_if_needed()
        except:
            return    
...
async def run():
    ...
    try:
        browser = await async_playwright_instance.chromium.connect_over_cdp(
            "http://localhost:9227"
        )
        ...
        for i, page in enumerate(pages):
            ...
            if i >= len(CONFIG["TRAIN_NUMBERS"]):
                print(f"No train assigned for tab {i}")
                continue

            train_number = CONFIG["TRAIN_NUMBERS"][i]

            tasks.append(
                asyncio.create_task(
                    run_page(page, train_number)
                )
            )
